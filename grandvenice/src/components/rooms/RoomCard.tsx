...
const badgeMap: Record<string, { label: string; class: string }> = {
  "standard-city":        { label: "Standard",        class: "bg-white/90 text-navy border border-stone-300/80" },
  "standard-twin":        { label: "Twin",            class: "bg-white/90 text-navy border border-stone-300/80" },
...
export function RoomCard({ room }: RoomCardProps) {
  const badge    = badgeMap[room.type] || { label: room.type, class: "bg-white/90 text-navy border border-stone-300/80" };
  ...
  return (
    <div className="luxury-card group relative flex flex-col overflow-hidden bg-stone-100 border border-stone-300/70">
      ...
      <div className="relative h-[300px] overflow-hidden flex-shrink-0">
        ...
        <div className="absolute inset-0 bg-sky-700/0 group-hover:bg-sky-700/12 transition-all duration-600" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 z-10">
          <span className="font-serif text-3xl text-white font-extralight leading-none">
            {formatNGN(room.pricePerNight)}
          </span>
          <span className="text-white/45 text-[10px] ml-1.5 font-sans tracking-wide">/ night</span>
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-all duration-500">
          <Link
            ...
            className="flex items-center gap-2 bg-sky-700 text-white text-[10px] tracking-[0.28em] uppercase px-7 py-3.5 font-semibold hover:bg-sky-600 transition-colors shadow-[0_4px_20px_rgba(26,111,168,0.5)]"
          >
        ...
        <div className="absolute top-3 right-3 w-7 h-7 border-t border-r border-white/25 z-10 pointer-events-none" />
        <div className="absolute bottom-3 right-3 w-7 h-7 border-b border-r border-white/25 z-10 pointer-events-none" />
      </div>

      {/* ── Content ── */}
      <div className="p-7 flex flex-col flex-1">
        <div className="flex items-start justify-between mb-4">
          <h3 className="font-serif text-2xl text-navy font-light leading-tight">
            {room.name}
          </h3>
        ...
        <div className="flex items-center gap-5 mb-5">
          {room.bedType && (
            <span className="flex items-center gap-1.5 text-[10px] text-slate tracking-widest font-sans uppercase">
        ...
        {room.shortDescription && (
          <p className="text-slate text-sm leading-[1.75] mb-6 flex-1 line-clamp-2 font-sans font-light">
            {room.shortDescription}
          </p>
        )}

        <div className="flex items-center justify-between pt-5 border-t border-stone-300/70 mt-auto">
          <Link
            href={`/booking?roomId=${room._id}&room=${encodeURIComponent(room.name)}&price=${room.pricePerNight}`}
            className="text-[10px] tracking-[0.28em] uppercase text-sky-700 font-sans font-semibold border-b border-sky-300 hover:border-sky-600 transition-all duration-300 pb-0.5"
          >
            Reserve Now
          </Link>
          <Link
            href={`/rooms/${room.slug?.current}`}
            className="text-[10px] tracking-[0.28em] uppercase text-slate/70 hover:text-navy transition-colors flex items-center gap-1.5 font-sans"
          >
